...
const taqnyatApiUrl =
  process.env.TAQNYAT_API_URL || "https://api.taqnyat.sa/v1/messages";
...
export class SmsService {
  ...
  private static async sendTaqnyatMessage(
    to: string,
    message: string,
  ): Promise<SmsResult> {
  ...
  private static async sendMessage(
    messageData: Omit<InsertSmsMessage, "id" | "twilioMessageId">,
  ): Promise<SmsMessage> {
    ...
    try {
      const result = await this.sendTaqnyatMessage(
