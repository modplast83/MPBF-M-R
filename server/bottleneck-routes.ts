...
export function setupBottleneckRoutes(app: Express) {
  ...
  app.get("/api/production/metrics", async (req, res) => {
    try {
      ...
      let metrics;
      if (section) {
        metrics = await bottleneckStorage.getProductionMetricsBySection(
          section as string,
        );
      } else if (machine) {
        metrics = await bottleneckStorage.getProductionMetricsByMachine(
          machine as string,
        );
  ...
  app.post("/api/production/metrics", async (req, res) => {
    try {
      ...
      const metric =
        await bottleneckStorage.createProductionMetric(validatedData);
      res.json(metric);
    } catch (error) {
  ...
  app.get("/api/bottleneck/alerts", async (req, res) => {
    try {
      ...
      if (status === "active") {
        ...
        alerts = await bottleneckStorage.getBottleneckAlertsBySection(
          section as string,
        );
      } else {
  ...
  app.post("/api/bottleneck/alerts", async (req, res) => {
    try {
      ...
      const alert =
        await bottleneckStorage.createBottleneckAlert(validatedData);
      res.json(alert);
    } catch (error) {
  ...
  app.get("/api/production/targets", async (req, res) => {
    try {
      ...
      let targets;
      if (section) {
        targets = await bottleneckStorage.getProductionTargetsBySection(
          section as string,
        );
      } else {
  ...
  app.post("/api/production/targets", async (req, res) => {
    try {
      ...
      const target =
        await bottleneckStorage.createProductionTarget(validatedData);
      res.json(target);
    } catch (error) {
  ...
  app.put("/api/production/targets/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const target = await bottleneckStorage.updateProductionTarget(
        id,
        req.body,
      );
  ...
  app.get("/api/notification/settings", async (req, res) => {
    try {
      ...
      let settings;
      if (userId) {
        settings = await bottleneckStorage.getNotificationSettingsByUser(
          userId as string,
        );
      } else {
  ...
  app.post("/api/notification/settings", async (req, res) => {
    try {
      ...
      const setting =
        await bottleneckStorage.createNotificationSetting(validatedData);
      res.json(setting);
    } catch (error) {
  ...
  app.put("/api/notification/settings/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const setting = await bottleneckStorage.updateNotificationSetting(
        id,
        req.body,
      );
  ...
  // Analytics endpoints
  app.get(
    "/api/production/analytics/efficiency/:sectionId",
    async (req, res) => {
      try {
        const sectionId = req.params.sectionId;
        const days = parseInt(req.query.days as string) || 7;

        const trends = await bottleneckStorage.getEfficiencyTrends(
          sectionId,
          days,
        );
        res.json(trends);
    ...
    try {
      ...
      const overallEfficiency =
        last24Hours.length > 0
          ? last24Hours.reduce((sum, m) => sum + m.efficiency, 0) /
            last24Hours.length
          : 0;
