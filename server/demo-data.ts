...
async function safeInitialize<T>(
  operation: () => Promise<T>,
  errorMessage: string,
): Promise<T | null> {
  try {
...
export async function initializeDemoData(storage: IStorage) {
  try {
    ...
    const existingHdpe = existingRawMaterials.find(
      (rm) => rm.name === "HDPE" && rm.type === "Plastic",
    );
    if (!existingHdpe) {
    ...
    const existingLdpe = existingRawMaterials.find(
      (rm) => rm.name === "LDPE" && rm.type === "Plastic",
    );
    if (!existingLdpe) {
      await storage.createRawMaterial({
        name: "LDPE",
    ...
    let customer1 = await safeInitialize(async () => {
    ...
    let customer2 = await safeInitialize(async () => {
    ...
    let product1 = existingProducts.find(
      (p) => p.customerId === customer1.id && p.itemId === smallBag.id,
    );
    if (!product1) {
    ...
    let product2 = existingProducts.find(
      (p) => p.customerId === customer1.id && p.itemId === mediumBag.id,
    );
    if (!product2) {
    ...
    let product3 = existingProducts.find(
      (p) => p.customerId === customer2.id && p.itemId === largeBag.id,
    );
    if (!product3) {
    ...
    let order1;
    let order1Id = -1;
    const existingOrder1 = existingOrders.find(
      (o) =>
        o.customerId === customer1.id && o.note === "Urgent delivery needed",
    );
    if (existingOrder1) {
    ...
    let jobOrder1;
    const existingJobOrder1 = existingJobOrders.find(
      (jo) => jo.orderId === order1Id && jo.customerProductId === product1.id,
    );
    ...
    let jobOrder2;
    const existingJobOrder2 = existingJobOrders.find(
      (jo) => jo.orderId === order1Id && jo.customerProductId === product2.id,
    );
    ...
    let order2;
    let order2Id = -1;
    const existingOrder2 = existingOrders.find(
      (o) =>
        o.customerId === customer2.id && o.note === "Monthly repeated order",
    );
    if (existingOrder2) {
    ...
    let jobOrder3;
    const existingJobOrder3 = existingJobOrders.find(
      (jo) => jo.orderId === order2Id && jo.customerProductId === product3.id,
    );
    ...
    const existingInk = existingRawMaterials.find(
      (rm) => rm.name === "Colored Ink" && rm.type === "Ink",
    );
    if (!existingInk) {
      await storage.createRawMaterial({
        name: "Colored Ink",
