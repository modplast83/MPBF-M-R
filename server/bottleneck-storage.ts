...
import {
  ProductionMetrics,
  InsertProductionMetrics,
  BottleneckAlert,
  InsertBottleneckAlert,
  NotificationSettings,
  InsertNotificationSettings,
  ProductionTargets,
  InsertProductionTargets,
} from "@shared/schema";
...
export class BottleneckStorage {
  ...
  async getProductionMetricsBySection(
    sectionId: string,
  ): Promise<ProductionMetrics[]> {
    return this.productionMetrics.filter(
      (metric) => metric.sectionId === sectionId,
    );
  }

  async getProductionMetricsByMachine(
    machineId: string,
  ): Promise<ProductionMetrics[]> {
    return this.productionMetrics.filter(
      (metric) => metric.machineId === machineId,
    );
  }
  ...
  async createProductionMetric(
    metric: InsertProductionMetrics,
  ): Promise<ProductionMetrics> {
  ...
  async getBottleneckAlerts(): Promise<BottleneckAlert[]> {
    return this.bottleneckAlerts.sort(
  ...
  async getBottleneckAlertsBySection(
    sectionId: string,
  ): Promise<BottleneckAlert[]> {
    return this.bottleneckAlerts.filter(
      (alert) => alert.sectionId === sectionId,
    );
  }
  ...
  async acknowledgeAlert(
    id: number,
    userId: string,
  ): Promise<BottleneckAlert | undefined> {
  ...
  async createBottleneckAlert(
    alert: InsertBottleneckAlert,
  ): Promise<BottleneckAlert> {
  ...
  async getProductionTargetsBySection(
    sectionId: string,
  ): Promise<ProductionTargets[]> {
    return this.productionTargets.filter(
      (target) => target.sectionId === sectionId && target.isActive,
    );
  }

  async createProductionTarget(
    target: InsertProductionTargets,
  ): Promise<ProductionTargets> {
  ...
  async updateProductionTarget(
    id: number,
    updates: Partial<ProductionTargets>,
  ): Promise<ProductionTargets | undefined> {
  ...
  async getNotificationSettingsByUser(
    ...
    return this.notificationSettings.filter(
      (setting) => setting.userId === userId,
    );
  }

  async createNotificationSetting(
    setting: InsertNotificationSettings,
  ): Promise<NotificationSettings> {
  ...
  private async analyzeForBottlenecks(
    ...
    const relevantTarget = targets.find(
      (t) =>
    ...
    if (metric.actualRate < relevantTarget.targetRate * 0.8) {
      alerts.push({
        ...
        severity:
          metric.actualRate < relevantTarget.targetRate * 0.5
            ? "critical"
            : "high",
        sectionId: metric.sectionId,
        machineId: metric.machineId,
        ...
        affectedJobOrders: metric.jobOrderId ? [metric.jobOrderId] : [],
        estimatedDelay: this.calculateDelayFromRate(
          metric.actualRate,
          relevantTarget.targetRate,
        ),
    ...
    if (metric.downtime && metric.downtime > relevantTarget.maxDowntime) {
      alerts.push({
        ...
        severity:
          metric.downtime > relevantTarget.maxDowntime * 2
            ? "critical"
            : "high",
        sectionId: metric.sectionId,
        machineId: metric.machineId,
  ...
  private calculateEstimatedDelay(
    ...
    if (
      actualEfficiency <= 0 ||
      targetEfficiency <= 0 ||
      !isFinite(actualEfficiency) ||
      !isFinite(targetEfficiency)
    )
      return 0;
  ...
  private calculateDelayFromRate(
    actualRate: number,
    targetRate: number,
  ): number {
    if (
  ...
  async getEfficiencyTrends(sectionId: string, days: number = 7): Promise<any> {
    ...
    const metrics = await this.getProductionMetricsByDateRange(
      startDate,
      new Date(),
    );
    ...
    return {
      averageEfficiency:
        sectionMetrics.length > 0
          ? Math.max(
              0,
              sectionMetrics.reduce((sum, m) => sum + (m.efficiency || 0), 0) /
                sectionMetrics.length,
            )
          : 0,
      dailyTrends: this.groupMetricsByDay(sectionMetrics),
      alertCount: this.bottleneckAlerts.filter(
        (a) => a.sectionId === sectionId && a.detectedAt >= startDate,
      ).length,
    };
  }
      ...
      averageEfficiency:
        day.metrics.length > 0
          ? Math.max(
              0,
              day.metrics.reduce(
                (sum: number, m: ProductionMetrics) =>
                  sum + (m.efficiency || 0),
                0,
              ) / day.metrics.length,
            )
          : 0,
      totalDowntime: day.metrics.reduce(
        (sum: number, m: ProductionMetrics) => sum + (m.downtime || 0),
        0,
      ),
      averageRate:
        day.metrics.length > 0
