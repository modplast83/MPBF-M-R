...
export class HybridStorage implements IStorage {
  ...
  async getCustomerProductsByCustomer(
    ...
    try {
      ...
      console.error(
        `Failed to get customer products by customer ${customerId}:`,
        error,
      );
      return [];
    }
  ...
  async deleteOrder(id: number): Promise<boolean> {
    try {
      ...
      const relatedJobOrders = await db
        .select()
        .from(jobOrders)
      ...
      if (relatedJobOrders.length > 0) {
        console.log(
          `Found ${relatedJobOrders.length} job orders related to order #${id}`,
        );
        ...
        if (relatedRolls.length > 0) {
          console.log(
            `Found ${relatedRolls.length} rolls related to job orders of order #${id}`,
          );
          ...
          if (rollIds.length > 0) {
            try {
              ...
              console.log(
                `Deleted quality checks related to rolls of order #${id}`,
              );
            } catch (error) {
              console.error(
                `Error deleting quality checks for order #${id}:`,
                error,
              );
            }
            ...
            try {
              ...
              console.log(
                `Deleted rolls related to job orders of order #${id}`,
              );
            } catch (error) {
        ...
        try {
          ...
          console.log(
            `Deleted final products related to job orders of order #${id}`,
          );
        } catch (error) {
          console.error(
            `Error deleting final products for order #${id}:`,
            error,
          );
        }
        ...
        try {
          ...
          if (smsTableExists) {
            ...
            if (jobOrderIds.length > 0) {
              ...
              await db.execute(
                `DELETE FROM sms_messages WHERE job_order_id IN (${jobOrderIds.join(",")})`,
              );
            }
            // Delete SMS messages related directly to the order
            await db.execute(`DELETE FROM sms_messages WHERE order_id = ${id}`);
            console.log(
              `Deleted SMS messages related to order #${id} and its job orders`,
            );
          } else {
            console.log(
              `SMS messages table doesn't exist yet, skipping SMS deletion for order #${id}`,
            );
          }
        } catch (error: any) {
        ...
        try {
          ...
          console.log(
            `Deleted quality checks related to job orders of order #${id}`,
          );
        } catch (error) {
          console.error(
            `Error deleting quality checks for job orders of order #${id}:`,
            error,
          );
        }
  ...
  async getQualityCheckTypesByStage(
    ...
    try {
      ...
      console.error(
        `Failed to get quality check types by stage ${stage}:`,
        error,
      );
      return [];
    }
  ...
  async getQualityCheckType(id: string): Promise<QualityCheckType | undefined> {
    try {
      const [qualityCheckType] = await db
        .select()
        .from(qualityCheckTypes)
  ...
  async getQualityChecksByJobOrder(
    ...
    try {
      ...
      console.error(
        `Failed to get quality checks by job order ${jobOrderId}:`,
        error,
      );
      return [];
    }
  ...
  async getCorrectiveActionsByQualityCheck(
    ...
    try {
      ...
      console.error(
        `Failed to get corrective actions by quality check ${qualityCheckId}:`,
        error,
      );
      return [];
    }
  ...
  async getSmsMessagesByJobOrder(jobOrderId: number): Promise<SmsMessage[]> {
    try {
      ...
      console.error(
        `Failed to get SMS messages by job order ${jobOrderId}:`,
        error,
      );
      return [];
    }
  ...
  async getSmsMessagesByCustomer(customerId: string): Promise<SmsMessage[]> {
    try {
      ...
      console.error(
        `Failed to get SMS messages by customer ${customerId}:`,
        error,
      );
      return [];
    }
  ...
  async createPlatePricingParameter(
    ...
    try {
      ...
      throw new Error(
        "Failed to create pricing parameter: " + (error as Error).message,
      );
    }
  }
