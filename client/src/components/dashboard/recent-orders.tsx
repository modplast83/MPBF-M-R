...
export function RecentOrders() {
  ...
  if (isLoading) {
    return (
      <Card
        className={`${isRTL ? "rtl text-right" : ""} rounded-xl overflow-hidden border border-gray-100`}
      >
        ...
        <CardContent className="p-0">
          <div className="divide-y divide-gray-100">
            {[...Array(isMobile ? 3 : 4)].map((_, i) => (
              <div
                key={i}
                className={`p-3 sm:p-4 animate-pulse flex items-center justify-between ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <div className="w-2/3">
  ...
  return (
    <Card
      className={`${isRTL ? "rtl text-right" : ""} rounded-xl overflow-hidden border border-gray-100 hover:shadow-md transition-all duration-200`}
    >
        ...
        <ul className="divide-y divide-gray-100">
          {recentOrders?.length ? (
            recentOrders.map((order) => (
              <li
                key={order.id}
                className={`p-3 sm:p-4 hover:bg-gray-50 transition-colors flex justify-between items-center ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <div className="min-w-0 flex-1 pr-2">
                  ...
                  <p className="text-sm text-gray-500 mt-1 flex items-center truncate">
                    <span className="material-icons text-gray-400 text-xs mr-1 flex-shrink-0">
                      business
                    </span>
            ...
            <li className="py-6 sm:py-8 text-center text-gray-500 bg-gray-50/50">
              {t("orders.no_recent_orders")}
            </li>
          )}
        </ul>
      </CardContent>
      <CardFooter
        className={`bg-gray-50/50 border-t border-gray-100 py-2 sm:py-3 ${isRTL ? "justify-end" : ""}`}
      >
        <Link
          href="/orders"
          className={`text-primary-600 text-sm font-medium flex items-center hover:text-primary-700 transition-colors ${isRTL ? "flex-row-reverse" : ""}`}
        >
          {t("orders.see_all_orders")}
          <span
            className={`material-icons ${isRTL ? "ml-0 mr-1" : "ml-1"} bg-primary-100 rounded-full h-5 w-5 flex items-center justify-center text-xs`}
          >
