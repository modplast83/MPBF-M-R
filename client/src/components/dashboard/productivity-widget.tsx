          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="border rounded-lg p-3">
                <div className="text-sm text-muted-foreground mb-1">
                  {t("dashboard.operator_efficiency")}
                </div>
                <div className="flex items-end gap-2">
                  <div
                    className={`text-2xl font-bold ${getPercentageColor(data.operatorEfficiency)}`}
                  >
              ...
              <div className="border rounded-lg p-3">
                <div className="text-sm text-muted-foreground mb-1">
                  {t("dashboard.machine_utilization")}
                </div>
                <div className="flex items-end gap-2">
                  <div
                    className={`text-2xl font-bold ${getPercentageColor(data.machineUtilization)}`}
                  >
            ...
            <div className="border rounded-lg p-4 space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {t("dashboard.units_per_hour")}
                  </div>
                </div>
                <div>
                  <div className="text-sm text-right">
                    {t("dashboard.target")}: {data.productionTarget}
                  </div>
                  <div
                    className={`text-xs text-right ${getPerformanceColor(data.productionPerHour, data.productionTarget)}`}
                  >
                    {t(
                      `dashboard.performance_${getPerformanceText(data.productionPerHour, data.productionTarget).toLowerCase()}`,
                    )}
                  </div>
                </div>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2.5">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (data.productionPerHour / data.productionTarget) * 100)}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div className="border rounded-lg p-3">
              <div className="flex justify-between items-center">
                <div>
                  <div className="text-sm text-muted-foreground">
                    {t("dashboard.cycle_time_variance")}
                  </div>
