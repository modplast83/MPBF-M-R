      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <div className="rounded-full bg-primary-50 p-2">
            <span className="material-icons text-primary-600">monitoring</span>
          </div>
        </div>
      </CardHeader>
      <CardContent>
    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-2">
          <div className={cn("rounded-md p-1.5 shadow-sm", iconBgColor)}>
            <span className={cn("material-icons text-sm", progressColor)}>
              {status.icon}
            </span>
          </div>
          <div>
      ...
      <Progress
        value={status.value}
        className={`h-2 ${getProgressClass(status.level)}`}
      />
    </div>
  );
}
...
function calculateProductionEfficiency(
  ...
  const efficiency =
    rolls.length > 0 ? (completedRolls / rolls.length) * 100 : 0;
