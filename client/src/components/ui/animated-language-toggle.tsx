...
export default function AnimatedLanguageToggle({
  ...
  if (variant === "button") {
    return (
      ...
      >
        <div
          className={`flex items-center gap-2 transition-transform duration-300 ${isAnimating ? "scale-95" : "scale-100"}`}
        >
      ...
      >
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span
                  className={`text-xs ${isSelected ? "text-primary-600" : "text-gray-500"}`}
                >
                  {lang.name}
                </span>
              </div>
        ...
        <div className="border-t my-1 pt-2">
          <div className="px-3 py-2 text-xs text-gray-500 flex items-center gap-2">
            ...
            <span>
              {t("language.change_interface", "Change interface language")}
            </span>
          </div>
        </div>
      </DropdownMenuContent>
    ...
    <div className={`relative ${className}`}>
      ...
      >
        ...
        <span>
          {context
            ? t(
                `language.viewing_in_${language}`,
                `Viewing ${context} in ${languages[language].nativeName}`,
              )
            : t(
                `language.interface_in_${language}`,
                `Interface in ${languages[language].nativeName}`,
              )}
        </span>
      </div>
