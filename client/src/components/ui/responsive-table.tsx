    ...
    if (titleColumn) {
      return titleColumn.cell
        ...
        : titleColumn.accessorKey &&
            typeof titleColumn.accessorKey !== "function"
  ...
  return (
    ...
    >
        ...
        >
          {searchable && (
            <div className="relative max-w-xs">
              ...
              />
              <div
                className={`absolute ${isRightToLeft ? "right-3" : "left-3"} top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none`}
              >
...

      {
        isMobile
          ? renderMobileView()
          : null /* Desktop table is rendered by parent */
      }

      {pagination && totalPages > 0 && isMobile && (
        <div
          className={`flex items-center justify-between pt-2 ${isRightToLeft ? "flex-row-reverse" : ""}`}
        >
          <div
            className={`text-xs text-gray-500 ${isRightToLeft ? "text-right" : ""}`}
          >
          ...
          <div
            className={`flex items-center ${isRightToLeft ? "space-x-reverse space-x-2 flex-row-reverse" : "space-x-2"}`}
          >
            <Button
              variant="outline"
              size="icon"
