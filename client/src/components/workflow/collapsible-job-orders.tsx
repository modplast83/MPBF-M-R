...
export function CollapsibleJobOrdersForExtrusion() {
  ...
  const createRollMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [`${API_ENDPOINTS.JOB_ORDERS}/${variables.jobOrderId}/rolls`],
      });
    ...
    onError: (error) => {
      toast({
        ...
        description: t("production.roll_management.roll_creation_failed", {
          error,
        }),
        variant: "destructive",
      });
  ...
  const calculateProgress = (jobOrder: JobOrder): number => {
    ...
    const jobOrderRolls = allRolls.filter(
    ...
    const totalExtrudedQty = jobOrderRolls.reduce(
    ...
    return Math.min(
      Math.round((totalExtrudedQty / jobOrder.quantity) * 100),
      100,
    );
  };
  ...
  const getMasterBatchName = (jobOrder: JobOrder): string => {
    if (!customerProducts.length || !masterBatches.length)
      return t("common.loading");

    const product = customerProducts.find(
      (cp) => cp.id === jobOrder.customerProductId,
    );
    if (!product || !product.masterBatchId)
      return t("production.no_master_batch");
    ...
    return masterBatch
      ? masterBatch.name
      : t("production.unknown_master_batch");
  };
  ...
  const filteredJobOrders = jobOrders.filter((job) => {
    ...
    return (
      ...
      parentOrder.status === "For Production"
    );
  });

  // Group job orders by order ID
  const groupedJobOrders = filteredJobOrders.reduce(
    (acc, job) => {
      ...
      {filteredJobOrders.length === 0 ? (
        <Card className="bg-white border border-dashed border-secondary-200">
          <CardContent className="py-6 text-center">
            <span className="material-icons text-3xl text-secondary-400 mb-2">
              assignment
            </span>
            <p className="text-secondary-500">
              {t("production.roll_management.no_job_orders_extrusion")}
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-6">
          {sortedGroupKeys.map((orderId, index) => {
            ...
            const customer = parentOrder
              ? customers.find((c) => c.id === parentOrder.customerId)
              : null;
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h4 className="text-left text-[19px] text-[#ff1212] font-bold">
                          Order #{orderId}
                        </h4>
                        <p className="text-secondary-600 truncate max-w-[200px] sm:max-w-[300px] font-bold text-[15px]">
                          {customerName}{" "}
                          {customerNameAr && (
                            <span className="font-bold mr-1 text-primary-700">
                              ({customerNameAr})
                            </span>
                          )}
                        </p>
                    ...
                    <div className="flex items-center">
                      <Badge className="bg-primary-100 text-primary-700 hover:bg-primary-200 text-xs mr-2">
                        {orderJobOrders.length} JO Counts
                      </Badge>
                      <span
                        className="material-icons text-primary-600 text-sm transition-transform duration-200"
                        style={{
                          transform: expandedOrderIds.includes(orderId)
                            ? "rotate(180deg)"
                            : "rotate(0deg)",
                        }}
                      >
                ...
                {expandedOrderIds.includes(orderId) && (
                  ...
                  >
                    {orderJobOrders.map((jobOrder) => {
                      ...
                      const jobOrderRolls = extrusionRolls.filter(
                        (roll) => roll.jobOrderId === jobOrderId,
                      );
                            ...
                            <div className="flex-1 flex flex-col sm:flex-row sm:items-center">
                              <div className="flex items-center mb-2 sm:mb-0 sm:flex-1">
                                <div className="rounded-full bg-secondary-200 w-7 h-7 flex items-center justify-center mr-3">
                                  <span className="material-icons text-xs text-secondary-700">
                            ...
                            <div className="px-4 py-3 bg-secondary-50">
                              ...
                              <div className="mb-3">
                                ...
                                {jobOrderRolls.length === 0 ? (
                                  <p className="text-xs text-secondary-500">
                                    {t(
                                      "production.roll_management.no_rolls_created",
                                    )}
                                  </p>
                                ) : (
                                      ...
                                      >
                                        <div>
                                          ...
                                          <p className="text-sm text-secondary-500">
                                            {t(
                                              "production.roll_management.quantity",
                                            )}
                                            : {roll.extrudingQty} kg
                                          </p>
                                        </div>
                                        <Badge
                                          variant={
                              ...
                              <div className="flex justify-end">
                                <AnimatedButton
                                  ...
                                  disabled={
                                    isComplete || createRollMutation.isPending
                                  }
                                  className="flex items-center text-xs sm:text-sm py-1 px-2 sm:py-2 sm:px-3 h-auto"
                                  size="sm"
                                >
                                  <span className="material-icons text-xs sm:text-sm mr-1">
                                    add
                                  </span>
