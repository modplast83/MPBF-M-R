...
export function RollDialog({
  ...
  const { data: existingRolls = [] } = useQuery<Roll[]>({
    queryKey: [
      jobOrder ? `${API_ENDPOINTS.JOB_ORDERS}/${jobOrder.id}/rolls` : null,
    ],
    enabled: !!jobOrder,
  });

  // Calculate remaining quantity
  const totalExtrudedQty = existingRolls.reduce(
    (total, roll) => total + (roll.extrudingQty || 0),
    0,
  );
  const remainingQty = jobOrder
    ? Math.max(0, jobOrder.quantity - totalExtrudedQty)
    : 0;

  // Schema for roll creation (allowing exceeding job order quantity)
  const formSchema = z.object({
    extrudingQty: z.coerce.number().positive(t("rolls.quantity_error")),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`sm:max-w-[425px] ${isRTL ? "rtl" : ""}`}>
        <DialogHeader>
            ...
            <Badge variant="outline" className={`${isRTL ? "mr-2" : "ml-2"}`}>
              {stage === "extrusion" && (
                <span className="flex items-center text-primary-500">
                  <span
                    className={`material-icons text-sm ${isRTL ? "ml-1" : "mr-1"}`}
                  >
                    merge_type
                  </span>
                  {t("rolls.extrusion")}
          ...
          <DialogDescription>
            Create a new roll for the selected job order during the extrusion
            process.
          </DialogDescription>
        </DialogHeader>
            ...
            >
              <div className="grid gap-4 py-2">
                ...
                <div className="grid gap-1 bg-secondary-50 p-3 rounded-md">
                  <div className="text-sm font-medium">
                    {t("job_orders.title")} {t("common.details")}
                  </div>
                  ...
                  {exceedsLimit && (
                    <div
                      className={`mt-2 text-xs text-warning-600 bg-warning-50 p-2 rounded border border-warning-200 flex items-center ${isRTL ? "space-x-reverse" : ""}`}
                    >
                      <span
                        className={`material-icons text-sm ${isRTL ? "ml-1" : "mr-1"}`}
                      >
                      ...
                      {t("rolls.quantity_exceeds", {
                        amount: excessAmount.toFixed(2),
                      })}
                    </div>
                  )}
                </div>
              ...
              <DialogFooter className={isRTL ? "flex-row-reverse" : ""}>
                ...
                <Button
                  type="submit"
                  disabled={
                    isLoading || Number(form.getValues().extrudingQty) <= 0
                  }
                >
