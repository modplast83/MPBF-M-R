...
export function AuthRedirect() {
  ...
  const { getIntendedUrl, clearIntendedUrl, shouldPreserveUrl } =
    useUrlPreservation();
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && isAuthPage) {
      ...
      if (intendedUrl && shouldPreserveUrl(intendedUrl)) {
        console.log(
          "User is authenticated, redirecting to intended page:",
          intendedUrl,
        );
        clearIntendedUrl();
        ...
        console.log(
          "User is authenticated but on auth page, redirecting to dashboard",
        );
        clearIntendedUrl();
        setLocation("/");
      }
      ...
      if (shouldPreserveUrl(location)) {
        console.log(
          "User is not authenticated, saving current page and redirecting to auth",
        );
        // URL preservation is handled by ProtectedRoute component
      } else {
        console.log(
          "User is not authenticated and not on auth page, redirecting to auth",
        );
      }
      setLocation("/auth");
    }
