...
export function SmsAnalytics({ messages }: SmsAnalyticsProps) {
  const analytics = useMemo(() => {
    ...
    const recentMessages = messages.filter(
      (m) =>
        m.sentAt &&
        isWithinInterval(new Date(m.sentAt), { start: last7Days, end: now }),
    );
    ...
    const messagesByPriority = messages.reduce(
      (acc, message) => {
    ...
    const messagesByDay = Array.from({ length: 7 }, (_, i) => {
      ...
      const dayMessages = messages.filter(
        (m) =>
          m.sentAt &&
          isWithinInterval(new Date(m.sentAt), {
    ...
    // Unique recipients
    const uniqueRecipients = new Set(messages.map((m) => m.recipientPhone))
      .size;

    // Average retry count
    const totalRetries = messages.reduce(
      (sum, m) => sum + (m.retryCount || 0),
      0,
    );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Overview of SMS messaging performance and statistics
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {Object.entries(analytics.messagesByCategory).map(
              ([category, count]) => (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Priority distribution of sent messages
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(analytics.messagesByPriority).map(
              ([priority, count]) => (
                <div
                  key={priority}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Message delivery trends over the past week
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${day.total > 0 ? (day.delivered / day.total) * 100 : 0}%`,
                        }}
                      />
                    </div>
                    <div className="text-xs text-gray-500 w-12">
                      {day.total > 0
                        ? `${((day.delivered / day.total) * 100).toFixed(0)}%`
                        : "0%"}
                    </div>
                  </div>
                </div>
            ...
            <div className="text-center p-4 border rounded-lg">
              <div className="text-2xl font-bold text-blue-600">
                {analytics.totalMessages > 0
                  ? (analytics.totalMessages / 30).toFixed(1)
                  : "0"}
              </div>
              <div className="text-sm text-gray-500">
                Messages per Day (30d avg)
              </div>
            </div>
