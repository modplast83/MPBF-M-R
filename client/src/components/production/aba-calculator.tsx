...
export function AbaCalculator({ onPrint }: AbaCalculatorProps) {
  ...
  const calculateAbaFormula = (jobOrder: JobOrder) => {
    ...
    const totalBatches =
      remainingQuantity > 0 ? fullBatchCount + 1 : fullBatchCount;
    ...
    const totalScrewA = batches.reduce(
      (sum, batch) => sum + batch.totals.screwA,
      0,
    );
    const totalScrewB = batches.reduce(
      (sum, batch) => sum + batch.totals.screwB,
      0,
    );
  ...
  const handlePrint = () => {
    ...
    if (calculationResult.batches && calculationResult.batches.length > 1) {
      batchesHtml = calculationResult.batches
        .map(
          (batch) => `
        ...
        <body>
          ...
          <h3>${
            calculationResult.batches && calculationResult.batches.length > 1
              ? t(
                  "production.aba_calculator.aba_summary",
                  "ABA Formula - Summary",
                )
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="border-t pt-4">
                  <h3 className="font-medium mb-2">
                    {calculationResult.batches &&
                    calculationResult.batches.length > 1
                      ? t(
                          "production.aba_calculator.aba_summary",
                          "ABA Formula - Summary",
                        )
                      : t(
                          "production.aba_calculator.aba_formula",
                          "ABA Formula",
                        )}
                  </h3>
                      ...
                      <div className="mt-6">
                        <div className="bg-amber-50 border border-amber-200 rounded-md p-3 mb-4">
                          <p className="text-amber-800 text-sm">
                            {t(
                              "production.aba_calculator.batch_note",
                        ...
                        {calculationResult.batches.map((batch, batchIndex) => (
                          <div
                            key={batchIndex}
                            className="mb-6 border rounded-md p-3"
                          >
                            <h4 className="font-medium mb-2 text-slate-700">
                              {t("production.aba_calculator.batch", "Batch")}{" "}
      ...
      {data.batches && data.batches.length > 1 && (
        <div>
          <p className="mt-4 mb-2 text-sm italic text-gray-700 border-t pt-2">
            Note: This quantity exceeds the maximum 900kg per mixing formula.
            The calculation has been divided into the following batches:
          </p>

          {data.batches.map((batch, batchIndex) => (
            <div key={batchIndex} className="mb-6">
              <h3 className="text-md font-medium mb-2">
                Batch {batch.batchNumber} - {batch.quantity} kg
              </h3>

              <table className="w-full border-collapse mb-4">
                <thead>
                  <tr>
                    <th className="p-2 border bg-red-200 text-center">
                      Screw A
                    </th>
                    <th className="p-2 border bg-yellow-100 text-center">
                      Raw Materials
                    </th>
                    <th className="p-2 border bg-orange-200 text-center">
                      Screw B
                    </th>
                    <th className="p-2 border bg-yellow-200 text-center">
                      A+B Kg
                    </th>
                    <th className="p-2 border bg-yellow-50 text-center">
                      A+B %
                    </th>
                    <th className="p-2 border text-center">A %</th>
                    <th className="p-2 border text-center">B %</th>
