...
export function MixDetails({ mixId, rawMaterials, onClose }: MixDetailsProps) {
  ...
  const canDelete =
    hasPermission("Mix Materials", "delete") && hasExtrudingSection;
  ...
  const handleRemoveMaterial = (id: number) => {
    if (
      confirm("Are you sure you want to remove this material from the mix?")
    ) {
  ...
  const generatePrintableLabel = () => {
    ...
    return (
      ...
      >
        ...
        >
          <div>
            {t("common.total")}: {mixItems.length}
          </div>
          <div>{mix.totalQuantity?.toFixed(2) || "0.00"} kg</div>
        </div>
        <div
          style={{ textAlign: "center", fontSize: "9pt", marginTop: "0.3in" }}
        >
  ...
  if (!mix) {
    return (
      <div className="text-center p-4 text-secondary-400">
        {t("common.not_found")}
      </div>
    );
  }
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <div className="font-medium text-secondary-500">
                    {t("production.mix_materials.mix_id")}
                  </div>
                  <div>{mix.id}</div>
                </div>
                <div>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("production.mix_materials.material")}
                      />
                    </SelectTrigger>
                    <SelectContent>
