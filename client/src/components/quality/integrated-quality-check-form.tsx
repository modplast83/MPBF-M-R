...
export function IntegratedQualityChecksManagement() {
  ...
  const [selectedCheckType, setSelectedCheckType] =
    useState<QualityCheckType | null>(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      apiRequest("DELETE", `/api/quality-checks/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/quality-checks"] });
      toast({
        title: t(
          "quality.check_deleted_success",
    ...
    if (checkType) {
      checkType.checklistItems?.forEach((item) => {
        checklistResults[item] =
          check.checklistResults?.includes(item) || false;
      });
  ...
  const handlePrint = (check: QualityCheck) => {
    ...
    const roll = Array.isArray(rolls)
      ? rolls.find((r: any) => r.id === check.rollId)
      : null;
    const jobOrder = Array.isArray(jobOrders)
      ? jobOrders.find((jo: any) => jo.id === check.jobOrderId)
      : null;
    const order =
      jobOrder && Array.isArray(orders)
        ? orders.find((o: any) => o.id === jobOrder.orderId)
        : null;
    const customer =
      order && Array.isArray(customers)
        ? customers.find((c: any) => c.id === order.customerId)
        : null;
    const customerProduct =
      jobOrder && Array.isArray(customerProducts)
        ? customerProducts.find(
            (cp: any) => cp.id === jobOrder.customerProductId,
          )
        : null;
    const item =
      customerProduct && Array.isArray(items)
        ? items.find((i: any) => i.id === customerProduct.itemId)
        : null;
    const user = Array.isArray(users)
      ? users.find((u: any) => u.id === check.performedBy)
      : null;
    ...
    const checklistSection =
      check.checklistResults && check.checklistResults.length > 0
        ? `
      <div class="section">
        <h3 class="section-title">Checklist Results</h3>
        <div class="checklist-grid">
          ${check.checklistResults
            .map(
              (item) => `
    ...
    const parametersSection =
      check.parameterValues && check.parameterValues.length > 0
        ? `
      <div class="section">
  ...
  const getJobOrderName = (jobOrderId: number) => {
    const jobOrder = Array.isArray(jobOrders)
      ? jobOrders.find((jo: any) => jo.id === jobOrderId)
      : null;
    ...
    const customer = Array.isArray(customers)
      ? customers.find((c: any) => c.id === order?.customerId)
      : null;
    const customerProduct = Array.isArray(customerProducts)
      ? customerProducts.find((cp: any) => cp.id === jobOrder.customerProductId)
      : null;
  ...
  const filteredChecks = checks.filter((check) => {
    ...
    const matchesStage =
      filterStage === "all" ||
      (checkType && checkType.targetStage === filterStage);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {t(
              "quality.quality_checks_desc",
              "Perform and track quality inspections using defined check types",
            )}
          </p>
        </div>
        ...
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {t(
                  "quality.add_check_desc",
                  "Create a new quality check using predefined check types",
                )}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue
                            placeholder={t(
                              "quality.select_check_type",
                              "Select check type",
                            )}
                          />
                        </SelectTrigger>
                        <SelectContent>
                          {Array.isArray(checkTypes) &&
                            checkTypes
                    ...
                    <div>
                      <Label htmlFor="timestamp">
                        {t("quality.check_date", "Check Date")} *
                      </Label>
                      <Input
                        type="date"
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="pending">
                            {t("quality.pending", "Pending")}
                          </SelectItem>
                          <SelectItem value="passed">
                            {t("quality.passed", "Passed")}
                          </SelectItem>
                          <SelectItem value="failed">
                            {t("quality.failed", "Failed")}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="rollId">
                        {t("quality.roll", "Roll")} (
                        {t("common.optional", "Optional")})
                      </Label>
                      <Select
                        value={formData.rollId || "none"}
                        onValueChange={(value) =>
                          setFormData((prev) => ({
                            ...prev,
                            rollId: value === "none" ? undefined : value,
                          }))
                        }
                      >
                        <SelectTrigger>
                    ...
                    <div>
                      <Label htmlFor="jobOrderId">
                        {t("quality.job_order", "Job Order")} (
                        {t("common.optional", "Optional")})
                      </Label>
                      <Select
                        value={formData.jobOrderId?.toString() || "none"}
                        onValueChange={(value) =>
                          setFormData((prev) => ({
                            ...prev,
                            jobOrderId:
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="none">
                            {t("quality.no_job_order", "No Job Order")}
                          </SelectItem>
                            ...
                            jobOrders
                              .filter((jobOrder) => {
                                ...
                                return (
                                  Array.isArray(rolls) &&
                                  rolls.some(
                                    (roll) => roll.jobOrderId === jobOrder.id,
                                  )
                                );
                              })
                              .map((jobOrder: any) => (
              ...
              {/* Dynamic Checklist */}
              {selectedCheckType &&
                selectedCheckType.checklistItems &&
                selectedCheckType.checklistItems.length > 0 && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                      ...
                      <CardDescription>
                        {t(
                          "quality.checklist_desc",
                          "Mark completed items from the quality check type",
                        )}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
              ...
              {/* Dynamic Parameters */}
              {selectedCheckType &&
                selectedCheckType.parameters &&
                selectedCheckType.parameters.length > 0 && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                      ...
                      <CardDescription>
                        {t(
                          "quality.parameters_desc",
                          "Enter values for quality parameters",
                        )}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>
                    {t("quality.additional_info", "Additional Information")}
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  {formData.status === "failed" && (
                    <div>
                      <Label>
                        {t("quality.issue_severity", "Issue Severity")}
                      </Label>
                      <RadioGroup
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          ...
                          <Label htmlFor="minor">
                            {t("quality.minor", "Minor")}
                          </Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem value="major" id="major" />
                          <Label htmlFor="major">
                            {t("quality.major", "Major")}
                          </Label>
                        </div>
                        <div className="flex items-center space-x-2">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex gap-2">
          <Select value={filterStatus} onValueChange={setFilterStatus}>
            ...
            <SelectContent>
              <SelectItem value="all">
                {t("quality.all_status", "All Status")}
              </SelectItem>
              <SelectItem value="pending">
                {t("quality.pending", "Pending")}
              </SelectItem>
          ...
          <Select value={filterStage} onValueChange={setFilterStage}>
            ...
            <SelectContent>
              <SelectItem value="all">
                {t("quality.all_stages", "All Stages")}
              </SelectItem>
              ...
              <TableBody>
                {filteredChecks.length === 0 ? (
                  ...
                  filteredChecks.map((check) => {
                    const checkType = checkTypes.find(
                      (ct) => ct.id === check.checkTypeId,
                    );
                    return (
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {t("quality.view_check", "View Quality Check")}
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p>
                    {format(new Date(currentCheck.timestamp), "MMM dd, yyyy")}
                  </p>
                </div>
              </div>

              {currentCheck.checklistResults &&
                currentCheck.checklistResults.length > 0 && (
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">
                      {t(
                        "quality.completed_items",
              ...
              {currentCheck.parameterValues &&
                currentCheck.parameterValues.length > 0 && (
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">
