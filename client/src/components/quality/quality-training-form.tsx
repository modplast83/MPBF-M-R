...
export default function QualityTrainingForm({
  isOpen,
  onClose,
  editingTraining,
}: QualityTrainingFormProps) {
  const { t } = useTranslation();
  const { isRTL } = useLanguage();
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [prerequisites, setPrerequisites] = useState<string[]>(
    editingTraining?.prerequisites || [],
  );
  const [learningObjectives, setLearningObjectives] = useState<string[]>(
    editingTraining?.learningObjectives || [],
  );
  ...
  const form = useForm<QualityTrainingFormData>({
    ...
    defaultValues: {
      ...
      scheduledDate: editingTraining?.scheduledDate
        ? new Date(editingTraining.scheduledDate)
        : new Date(),
  ...
  const createTrainingMutation = useMutation({
    mutationFn: async (data: QualityTrainingFormData) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Failed to ${editingTraining ? "update" : "create"} training`,
        );
      }
    ...
    onError: (error) => {
      toast({
        ...
        description:
          error.message ||
          `Failed to ${editingTraining ? "update" : "create"} training. Please try again.`,
      });
    },
  });
      ...
      >
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {editingTraining
              ? "Edit Quality Training"
              : "Create Quality Training"}
          </DialogTitle>
          <DialogDescription>
            Create comprehensive quality training sessions to improve team
                    ...
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                      <div className="space-y-0.5">
                        ...
                        <div className="text-sm text-muted-foreground">
                          Participants must pass assessment to receive
                          certification
                        </div>
                      </div>
                      <FormControl>
