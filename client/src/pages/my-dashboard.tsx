...
export default function MyDashboard() {
  ...
  const { data: attendanceData = [] } = useQuery({
    ...
    queryFn: () =>
      apiRequest("GET", `/api/hr/time-attendance?userId=${user?.id}`),
    enabled: !!user?.id,
  });
  ...
  const { data: maintenanceRequests = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allRequests.filter(
        (r: MaintenanceRequest) => r.reportedBy === user?.id,
      );
    },
    enabled: !!user?.id,
  });
  ...
  const createMaintenanceRequestMutation = useMutation({
    mutationFn: (data: any) =>
      apiRequest("POST", "/api/maintenance-requests", {
        ...data,
        reportedBy: user?.id,
        requestedBy: user?.id,
      }),
    ...
    onError: (error: any) => {
      toast({
        title: t("common.error"),
        description:
          error.message || t("my_dashboard.failed_to_submit_maintenance"),
        variant: "destructive",
      });
    },
  });
  ...
  const calculateStats = () => {
    ...
    const totalWorkingHours = filteredAttendance.reduce(
      (sum: number, att: AttendanceRecord) => sum + (att.workingHours || 0),
      0,
    );
    const totalOvertimeHours = filteredAttendance.reduce(
      (sum: number, att: AttendanceRecord) => sum + (att.overtimeHours || 0),
      0,
    );
    const presentDays = filteredAttendance.filter(
      (att: AttendanceRecord) => att.status === "present",
    ).length;
    const absentDays = filteredAttendance.filter(
      (att: AttendanceRecord) => att.status === "absent",
    ).length;
    ...
    return {
      ...
      averageWorkingHours:
        filteredAttendance.length > 0
          ? totalWorkingHours / filteredAttendance.length
          : 0,
      presentDays,
      absentDays,
  ...
  const getCurrentWorkStatus = () => {
    if (!todayAttendance) {
      return {
        ...
        message: t("my_dashboard.ready_to_check_in"),
        color: "text-gray-500",
      };
    }
    ...
    if (todayAttendance.checkOutTime) {
      return {
        status: "checked_out",
        message: t("my_dashboard.work_day_completed"),
    ...
    if (todayAttendance.breakStartTime && !todayAttendance.breakEndTime) {
      return {
        status: "on_break",
        message: t("my_dashboard.on_break"),
    ...
    if (todayAttendance.checkInTime) {
      return {
        status: "working",
        message: t("my_dashboard.currently_working"),
        color: "text-blue-600",
      };
    }
    ...
    return {
      status: "unknown",
      message: t("my_dashboard.status_unknown"),
      color: "text-gray-500",
    };
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("my_dashboard.page_header_title", {
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center space-x-3">
              <div
                className={`w-3 h-3 rounded-full ${workStatus.status === "working" ? "bg-green-500" : workStatus.status === "on_break" ? "bg-orange-500" : "bg-gray-400"}`}
              />
              <div>
                <div className={`font-medium ${workStatus.color}`}>
                  {workStatus.message}
                </div>
                <div className="text-sm text-gray-500">
                  {t("my_dashboard.work_status")}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="font-medium">
                  {user?.sectionId || t("my_dashboard.not_assigned")}
                </div>
                <div className="text-sm text-gray-500">
                  {t("my_dashboard.section")}
                </div>
              </div>
            </div>
          </div>
      ...
      <Tabs defaultValue="attendance" className="space-y-4">
        <TabsList className="grid w-full grid-cols-5">
          <TabsTrigger value="attendance">
            {t("my_dashboard.tabs.attendance")}
          </TabsTrigger>
          <TabsTrigger value="violations">
            {t("my_dashboard.tabs.violations")}
          </TabsTrigger>
          <TabsTrigger value="trainings">
            {t("my_dashboard.tabs.trainings")}
          </TabsTrigger>
          <TabsTrigger value="maintenance">
            {t("my_dashboard.tabs.maintenance")}
          </TabsTrigger>
          <TabsTrigger value="overview">
            {t("my_dashboard.tabs.overview")}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="violations" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Your violation records and status
              </CardDescription>
            </CardHeader>
            <CardContent>
              {violations.length === 0 ? (
                  ...
                  {violations.map((violation: Violation) => (
                    <div key={violation.id} className="border rounded-lg p-4">
                      ...
                      <div className="text-sm text-gray-500 mb-2">
                        Incident Date:{" "}
                        {format(
                          new Date(violation.incidentDate),
                          "MMM dd, yyyy",
                        )}
                      </div>
                      <div className="text-sm">
        ...
        <TabsContent value="trainings" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Your training records and certifications
              </CardDescription>
            </CardHeader>
            <CardContent>
              {trainings.length === 0 ? (
            ...
            <CardContent>
              {maintenanceRequests.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500">
                    Submit a request when you need maintenance support
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setMaintenanceForm({ ...maintenanceForm, machineId: value })
                }
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setMaintenanceForm({ ...maintenanceForm, severity: value })
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button
                onClick={() =>
                  createMaintenanceRequestMutation.mutate(maintenanceForm)
                }
                disabled={
