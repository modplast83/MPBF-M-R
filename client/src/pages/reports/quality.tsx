...
export default function QualityReportsPage() {
  ...
  const [selectedCheck, setSelectedCheck] = useState<
    QualityReportData["checks"][0] | null
  >(null);
  ...
  const prepareQualityByStageData = () => {
    ...
    const stageData = reportData.checks.reduce(
      (acc: { [key: string]: { passed: number; failed: number } }, check) => {
  ...
  const prepareQualityTrendData = () => {
    ...
    const checksByDate = reportData.checks.reduce(
      (acc: { [key: string]: { passed: number; failed: number } }, check) => {
                ...
                <SelectContent>
                  ...
                  {reportData &&
                    Array.from(
                      ...
                      (rollId) =>
                        rollId && (
                          <SelectItem key={rollId} value={rollId}>
                            {rollId}
                          </SelectItem>
                        ),
                    )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <Progress value={reportData.passRate} className="h-2 mt-4">
                  <div
              ...
              {reportData.checks.length > 0 ? (
                <>
                  ...
                  <Card className="mb-8">
                    <CardHeader className="pb-0">
                      <CardTitle className="text-base">
                        {t("reports.pass_fail_distribution")}
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                                  ...
                                  <div className="grid grid-cols-2 gap-3">
                                    <div>
                                      ...
                                      <p className="text-sm font-medium">
                                        {formatDateString(check.date)}
                                      </p>
                                    </div>
                                    <div>
                                      <p className="text-xs text-gray-500">
                                        {t("reports.check_type")}
                                      </p>
          ...
          {selectedCheck && (
            <Card className="mt-6">
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle>
                  {t("reports.quality_check_details")} #{selectedCheck.id}
                </CardTitle>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <CardContent>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p>
                      {selectedCheck.roll
                        ? selectedCheck.roll.serialNumber
                        : "-"}
                    </p>
                  </div>
                </div>
                ...
                <h4 className="font-medium mb-3">
                  {t("reports.corrective_actions")}
                </h4>
                {selectedCheck.correctiveActions.length === 0 ? (
                  <p className="text-sm text-gray-500">
                    {t("reports.no_corrective_actions")}
                  </p>
                ) : (
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50">
                          ...
                          <th className="py-2 px-4 text-left font-medium">
                            {t("reports.implemented_by")}
                          </th>
                          <th className="py-2 px-4 text-left font-medium">
                            {t("reports.implementation_date")}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {selectedCheck.correctiveActions.map(
                          (action, index) => (
                            <tr
                              key={action.id}
                            ...
                            >
                              ...
                              <td className="py-2 px-4">
                                {formatDateString(action.implementationDate)}
                              </td>
                            </tr>
                          ),
                        )}
