...
export default function ProductionReportsPage() {
  ...
  const [selectedReport, setSelectedReport] =
    useState<ProductionReportItem | null>(null);
  ...
  const calculateSummaryMetrics = () => {
    ...
    const totalQuantity = reportData.reduce(
      (sum, report) => sum + report.metrics.totalQuantity,
      0,
    );
    const completedQuantity = reportData.reduce(
      (sum, report) => sum + report.metrics.completedQuantity,
      0,
    );

    // Calculate overall waste
    const totalWaste = reportData.reduce(
      (sum, report) => sum + report.metrics.wastageQuantity,
      0,
    );
    const totalExtrusion = reportData.reduce(
      (sum, report) => sum + report.metrics.extrusionQuantity,
      0,
    );
    const wastePercentage =
      totalExtrusion > 0 ? (totalWaste / totalExtrusion) * 100 : 0;

    // Calculate average efficiency
    const totalEfficiency = reportData.reduce(
      (sum, report) => sum + report.metrics.efficiency,
      0,
    );
  ...
  const prepareStatusChartData = () => {
    ...
    const statusCounts = reportData.reduce(
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters({
                    ...filters,
                    customerId: value === "all_customers" ? "" : value,
                  })
                }
              >
                <SelectTrigger>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">
                  {t("reports.total_production")}
                </p>
                <h3 className="text-2xl font-bold">
                  {formatNumber(summaryMetrics.totalQuantity, 1)} kg
                </h3>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">
                  {t("reports.waste_percentage")}
                </p>
                <h3 className="text-2xl font-bold">
                  {formatNumber(summaryMetrics.wastePercentage, 1)}%
                </h3>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">
                  {t("reports.avg_efficiency")}
                </p>
                <h3 className="text-2xl font-bold">
                  {formatNumber(summaryMetrics.avgEfficiency, 1)}%
                </h3>
              </div>
          ...
          {reportData && reportData.length > 0 ? (
            <>
              ...
              <Card className="mb-8">
                <CardHeader className="pb-0">
                  <CardTitle className="text-base">
                    {t("reports.efficiency_waste_trends")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="h-64">
              ...
              <div>
                <h3 className="text-lg font-semibold mb-4">
                  {t("reports.production_report_data")}
                </h3>
                ...
                {reportError ? (
                  ...
                  <div className="p-6 text-center">
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      {t("reports.try_different_filters")}
                    </p>
                  </div>
                ) : (
                  <>
                          ...
                          <Card key={report.id} className="overflow-hidden">
                            ...
                            <CardContent className="p-4">
                              <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                  <p className="text-xs text-gray-500">
                                    {t("reports.ordered_quantity")}
                                  </p>
                                  <p className="text-sm font-medium">
                                    {formatNumber(
                                      report.metrics.totalQuantity,
                                      1,
                                    )}{" "}
                                    kg
                                  </p>
                                ...
                                <div>
                                  <p className="text-xs text-gray-500">
                                    {t("reports.completed_quantity")}
                                  </p>
                                  <p className="text-sm font-medium">
                                    {formatNumber(
                                      report.metrics.completedQuantity,
                                      1,
                                    )}{" "}
                                    kg
                                  </p>
                                ...
                                <div>
                                  ...
                                  <p className="text-sm font-medium">
                                    {formatNumber(
                                      report.metrics.wastePercentage,
                                      1,
                                    )}
                                    %
                                  </p>
                                ...
                                <div>
                                  ...
                                  <p className="text-sm font-medium">
                                    {formatNumber(report.metrics.efficiency, 1)}
                                    %
                                  </p>
                                </div>
                              </div>
      ...
      {selectedReport && (
        <Card className="mt-6">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>
              {t("reports.order_details")} #{selectedReport.id}
            </CardTitle>
            <Button
              variant="ghost"
              size="sm"
                  ...
                  {selectedReport.products.map((product, index) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-4">
                        {formatNumber(product.quantity, 1)}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <CardContent className="p-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-semibold">
                      {formatNumber(selectedReport.metrics.totalQuantity, 1)} kg
                    </span>
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-semibold">
                      {formatNumber(
                        selectedReport.metrics.completedQuantity,
                        1,
                      )}{" "}
                      kg
                    </span>
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-semibold">
                      {formatNumber(selectedReport.metrics.efficiency, 1)}%
                    </span>
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                  <div className="flex flex-col">
                    <span className="text-sm text-gray-500">
                      {t("reports.extrusion_quantity")}
                    </span>
                    <span className="text-lg font-semibold">
                      {formatNumber(
                        selectedReport.metrics.extrusionQuantity,
                        1,
                      )}{" "}
                      kg
                    </span>
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex flex-col">
                    <span className="text-sm text-gray-500">
                      {t("reports.printing_quantity")}
                    </span>
                    <span className="text-lg font-semibold">
                      {formatNumber(selectedReport.metrics.printingQuantity, 1)}{" "}
                      kg
                    </span>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-semibold">
                      {formatNumber(selectedReport.metrics.wastePercentage, 1)}%
                    </span>
                  </div>
                </CardContent>
              </Card>
                ...
                <tbody>
                  {selectedReport.jobOrders.map((jobOrder, index) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-4">
                        {formatNumber(jobOrder.quantity, 1)}
                      </td>
                      <td className="py-2 px-4">
                        <Badge
                          className={
                      ...
                      <td className="py-2 px-4">
                        {jobOrder.completedDate
                          ? formatDateString(jobOrder.completedDate.toString())
                          : "-"}
                      </td>
                    </tr>
                  ))}
