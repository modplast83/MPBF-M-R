...
export default function WarehouseReportsPage() {
  ...
  const calculateSummaryMetrics = () => {
    ...
    const totalQuantity = reportData.currentInventory.reduce(
    ...
    const uniqueTypes = new Set(
      reportData.currentInventory.map((material) => material.type),
    );
    ...
    const recentUpdates = reportData.inventoryHistory.filter(
      (update) => new Date(update.date) >= oneWeekAgo,
    ).length;
  ...
  const prepareInventoryTypeChartData = () => {
    ...
    const inventoryByType = reportData.currentInventory.reduce(
      (acc: { [key: string]: number }, material) => {
  ...
  const prepareInventoryHistoryData = () => {
    ...
    const historyByDate = reportData.inventoryHistory.reduce(
      (acc: { [key: string]: number }, history) => {
        const dateStr = new Date(history.date).toISOString().split("T")[0];

        // Sum up the quantities of all materials in this input
        const totalQuantity = history.materials.reduce(
  ...
  const historyColumns = [
    ...
    },
    {
      ...
      cell: (row: any) => {
        const total = row.materials.reduce(
          (sum: number, material: any) => sum + material.quantity,
          0,
        );
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters({
                    ...filters,
                    materialType: value === "all_types" ? "" : value,
                  })
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  {reportData &&
                    Array.from(
                      new Set(
                        reportData.currentInventory.map((item) => item.type),
                      ),
                    ).map((type) => (
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters({
                    ...filters,
                    materialId: value === "all_materials" ? "" : value,
                  })
                }
              >
                <SelectTrigger>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">
                  {t("reports.total_materials")}
                </p>
                <h3 className="text-2xl font-bold">
                  {summaryMetrics.totalMaterials}
                </h3>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">
                  {t("reports.material_types")}
                </p>
                <h3 className="text-2xl font-bold">
                  {summaryMetrics.materialTypes}
                </h3>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">
                  {t("reports.recent_updates")}
                </p>
                <h3 className="text-2xl font-bold">
                  {summaryMetrics.recentUpdates}
                </h3>
              </div>
                              ...
                              <CardContent className="p-4">
                                <div className="grid grid-cols-2 gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm font-medium">
                                      {formatNumber(material.quantity || 0, 1)}{" "}
                                      {material.unit}
                                    </p>
                                  </div>
