...
function LineChart({ isMobile = false }: { isMobile?: boolean }) {
  ...
  return (
    <div
      className={`${isMobile ? "h-60" : "h-80"} flex items-center justify-center bg-secondary-50 rounded border border-dashed border-secondary-200`}
    >
      <div className="text-center px-4">
        <span className="material-icons text-4xl text-secondary-300">
          insert_chart
        </span>
...
export default function ReportsIndex() {
  ...
  const getProductionReportData = () => {
    const filteredOrders =
      orders?.filter((order) => filterByDateRange(order.date)) || [];

    return filteredOrders.map((order) => {
      const orderJobOrders =
        jobOrders?.filter((jo) => jo.orderId === order.id) || [];
      const totalQuantity = orderJobOrders.reduce(
      ...
      const productData = firstJobOrder
        ? getProductData(firstJobOrder.id)
        : { name: "N/A", size: "N/A" };
      ...
      return {
        ...
        efficiency:
          totalQuantity > 0
            ? ((completedQuantity / totalQuantity) * 100).toFixed(1) + "%"
            : "0%",
        status: order.status,
      };
    });
  ...
  const reportData =
    reportType === "production" ? getProductionReportData() : []; // For now only production report is implemented
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">
                    {formatNumber(row.completed, 1)} Kg
                  </p>
                </div>
              </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <Card className="h-full hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="flex items-center justify-center h-32 bg-orange-50 rounded mb-3">
                <span className="material-icons text-4xl text-orange-400">
          ...
          <Card className="h-full hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="flex items-center justify-center h-32 bg-green-50 rounded mb-3">
                <span className="material-icons text-4xl text-green-400">
                  inventory_2
                </span>
              </div>
              <p className="text-sm text-gray-600">
          ...
          <Card className="h-full hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="flex items-center justify-center h-32 bg-purple-50 rounded mb-3">
                <span className="material-icons text-4xl text-purple-400">
                  verified
                </span>
              </div>
              <p className="text-sm text-gray-600">
        ...
        >
          <Card className="h-full hover:shadow-md transition-shadow">
            ...
            <CardContent>
              ...
              <p className="text-sm text-gray-600">
                Analyze completed JO Mix production by kg with material, job
                order, and user filtering
              </p>
            </CardContent>
          </Card>
        </Link>
        ...
        <Link
          href="/reports/customer-info-report"
          className="block no-underline text-current"
        >
          <Card className="h-full hover:shadow-md transition-shadow">
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <div className="flex items-center justify-center h-32 bg-teal-50 rounded mb-3">
                <span className="material-icons text-4xl text-teal-400">
                  business
                </span>
              </div>
              <p className="text-sm text-gray-600">
                View, print, and manage customer registration information with
        ...
        <Card className="h-full">
          ...
          <CardContent>
            <div className="flex items-center justify-center h-32 bg-cyan-50 rounded mb-3">
              <span className="material-icons text-4xl text-cyan-400">
                show_chart
              </span>
            </div>
            <p className="text-sm text-gray-600">
        ...
        <Card className="h-full">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg">
              {t("reports.cost_analysis")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-center h-32 bg-amber-50 rounded mb-3">
              <span className="material-icons text-4xl text-amber-400">
                attach_money
              </span>
            </div>
            <p className="text-sm text-gray-600">
        ...
        <CardContent>
          ...
          <div>
            ...
            {isMobile ? (
              ...
              <DataTable data={reportData} columns={productionColumns as any} />
            )}
          </div>

          <div
            className={`flex ${isMobile ? "flex-col space-y-2" : "justify-end"} mt-6`}
          >
            <Button
              variant="outline"
