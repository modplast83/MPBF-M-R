...
function PerformanceCharts({
  ...
  const wasteData = [
    ...
    {
      name: t("performance.good_product"),
      value:
        data.wasteMetrics.overallWastePercentage > 0
          ? data.wasteMetrics.totalWasteQty /
              (data.wasteMetrics.overallWastePercentage / 100) -
            data.wasteMetrics.totalWasteQty
          : 0,
    },
  ];
  ...
  const qualityData = [
    {
      name: t("performance.passed"),
      value:
        data.qualityMetrics.totalQualityChecks -
        data.qualityMetrics.failedQualityChecks,
    },
    {
      name: t("performance.failed"),
      value: data.qualityMetrics.failedQualityChecks,
    },
  ];

  // Format processing times data for line chart
  const processingTimesData = data.mobileMetrics.recentProcessingTimes
    .map((item) => ({
      rollId: item.rollId,
  ...
  const throughputData = data.throughput.slice(-10).map((item) => {
    ...
    const dateFormat =
      dateParts.length >= 3 ? `${dateParts[2]}/${dateParts[1]}` : item.date;
    return {
      date: dateFormat,
      count: item.count,
      ...
      <Tabs defaultValue="processing" className="w-full">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">
                {t("performance.recent_processing_times")}
              </CardTitle>
            </CardHeader>
            <CardContent>
            ...
            <CardContent>
              ...
              <div className="mt-4">
                <div className="flex justify-between items-center mb-1 text-sm">
                  ...
                  <span>
                    {(100 - data.qualityMetrics.qualityFailureRate).toFixed(1)}%
                  </span>
                </div>
                <Progress
        ...
        <TabsContent value="throughput" className="mt-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">
                {t("performance.daily_throughput")}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="h-60 w-full">
                  ...
                  >
                    ...
                    <Tooltip
                      formatter={(value: any, name: any) => [
                        value,
                        name === "count"
                          ? t("performance.rolls_count")
                          : t("performance.weight_tons"),
                      ]}
                    />
                    <Legend />
                    <Bar
        ...
        <CardContent>
          ...
          <div className="mt-4">
            <div className="flex justify-between items-center mb-1 text-sm">
              ...
              <span>
                {data.wasteMetrics.overallWastePercentage.toFixed(1)}%
              </span>
            </div>
            <Progress
