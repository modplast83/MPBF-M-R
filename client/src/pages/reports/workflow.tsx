...
export default function WorkflowReportsPage() {
  ...
  const exportToExcel = () => {
    ...
    if (
      ...
      filteredReportData.sections?.length > 0
    ) {
    ...
    if (
      exportConfig.includeOperators &&
      filteredReportData.operators?.length > 0
    ) {
    ...
    const summaryData = [
      ...
      },
      {
        ...
        Value: filteredReportData.sections?.length || 0,
      },
      {
        Metric: "Total Operators",
        Value: filteredReportData.operators?.length || 0,
      },
  ...
  const exportToPDF = () => {
    ...
    if (
      ...
      const operatorsTableData = filteredReportData.operators.map(
        (operator) => {
          const user = users?.find((u) => u.id === operator.id);
          const section = sections?.find((s) => s.id === user?.sectionId);
          return [
  ...
  const exportToCSV = () => {
    ...
    if (
      ...
      csvContent += "SECTIONS\n";
      csvContent +=
        "Section ID,Section Name,Rolls Count,Total Quantity (kg),Waste Quantity (kg),Waste Percentage,Efficiency,Production Time (hours)\n";
    ...
    if (
      ...
      csvContent +=
        "Operator ID,Operator Name,Section,Jobs Count,Rolls Processed,Total Quantity (kg),Production Time (hours),Efficiency\n";
    ...
    if (exportConfig.includeItems && filteredReportData.items?.length > 0) {
      csvContent += "ITEMS\n";
      csvContent +=
        "Item ID,Item Name,Category,Jobs Count,Total Quantity (kg),Finished Quantity (kg),Waste Quantity (kg),Waste Percentage\n";
  ...
  const filteredUsers =
    users?.filter((user) => {
      ...
      return (
        user.sectionId &&
        filteredSections.some((section) => section.id === user.sectionId)
      );
    }) || [];
  ...
  const filteredReportData = reportData
    ? {
        ...reportData,
        sections:
          reportData.sections?.filter((section) =>
            allowedSections.some((allowed) =>
              section.name.toLowerCase().includes(allowed.toLowerCase()),
            ),
          ) || [],
        operators:
          reportData.operators?.filter((operator) => {
  ...
  const sectionsColumns = [
    ...
    },
    {
      ...
      cell: (row: any) =>
        `${formatNumber(row.productionTime, 1)} ${t("performance.hours")}`,
    },
  ];
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Showing data for production sections: Extruding, Printing, and
            Cutting only
          </p>
        </div>
        <PDFExportButton
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters({
                    ...filters,
                    sectionId: value === "all_sections" ? "" : value,
                  })
                }
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters({
                    ...filters,
                    operatorId: value === "all_operators" ? "" : value,
                  })
                }
              >
                <SelectTrigger>
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                      ...
                      >
                        ...
                        <Bar
                          ...
                          name={t("reports.total_quantity")}
                          fill="#2196F3"
                        />
                        <Bar
                          yAxisId="left"
                          dataKey="wasteQuantity"
                          name={t("reports.waste_quantity")}
                          fill="#F44336"
                        />
                        <Bar
                          yAxisId="right"
            ...
            <TabsContent value="operators" className="py-2">
              ...
              filteredReportData.operators.length > 0 ? (
                <>
                  <div className="mb-6">
                    <h3 className="text-lg font-medium mb-3">
                      {t("reports.operator_performance")}
                    </h3>
                    <ResponsiveContainer width="100%" height={300}>
                      <BarChart
                      ...
                      >
                        ...
                        <Bar
                          yAxisId="left"
                          dataKey="rollsProcessed"
                          name={t("reports.rolls_processed")}
                          fill="#FF9800"
                        />
                        <Bar
                          yAxisId="right"
                  ...
                  <div className="mb-6">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                      ...
                      >
                        ...
                        />
                        <Bar
                          dataKey="finishedQuantity"
                          name={t("reports.finished_quantity")}
                          fill="#4CAF50"
                        />
                        <Bar
                          dataKey="wasteQuantity"
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Label
                          ...
                          className="flex items-center cursor-pointer"
                        >
                          <span className="material-icons text-red-600 text-sm mr-1">
                            picture_as_pdf
                          </span>
                          PDF (.pdf)
                        </Label>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Label
                          ...
                          className="flex items-center cursor-pointer"
                        >
                          <span className="material-icons text-blue-600 text-sm mr-1">
                            description
                          </span>
                          CSV (.csv)
                        </Label>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor="sections" className="cursor-pointer">
                          {t("reports.sections_data")}
                        </Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setExportConfig((prev) => ({
                              ...prev,
                              includeOperators: !!checked,
                            }))
                          }
                        />
                        <Label htmlFor="operators" className="cursor-pointer">
                          {t("reports.operators_data")}
                        </Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Checkbox
