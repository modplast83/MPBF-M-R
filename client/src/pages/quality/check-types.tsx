...
export default function QualityCheckTypes() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: CheckTypeForm) =>
      apiRequest("POST", "/api/quality-check-types", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/quality-check-types"] });
      toast({
        title: t(
          "quality.check_type_created_success",
          "Quality check type created successfully",
        ),
      });
      setShowForm(false);
      form.reset();
    ...
    onError: () => {
      toast({
        title: t(
          "quality.check_type_created_error",
          "Failed to create quality check type",
        ),
        variant: "destructive",
      });
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: t(
          "quality.check_type_updated_success",
          "Quality check type updated successfully",
        ),
      });
      setShowForm(false);
      setEditingType(null);
    ...
    onError: () => {
      toast({
        title: t(
          "quality.check_type_updated_error",
          "Failed to update quality check type",
        ),
        variant: "destructive",
      });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      apiRequest("DELETE", `/api/quality-check-types/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/quality-check-types"] });
      toast({
        title: t(
          "quality.check_type_deleted_success",
          "Quality check type deleted successfully",
        ),
      });
    },
    onError: () => {
      toast({
        title: t(
          "quality.check_type_deleted_error",
          "Failed to delete quality check type",
        ),
        variant: "destructive",
      });
    },
  ...
  const filteredCheckTypes = checkTypes.filter((checkType) => {
    const matchesSearch =
      checkType.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (checkType.description
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ??
        false);
    const matchesStage =
      stageFilter === "all" || checkType.targetStage === stageFilter;
  ...
  const handleDelete = (checkType: QualityCheckType) => {
    if (
      window.confirm(
        t("quality.delete_check_type_confirm", { name: checkType.name }),
      )
    ) {
  ...
  return (
    <div className={`container mx-auto py-6 ${isRTL ? "rtl" : ""}`}>
      <PageHeader
        ...
        description={t(
          "quality.check_types_desc",
          "Define quality check templates for different stages",
        )}
      />
        ...
        <div className="flex gap-4 flex-1">
          ...
          <Select value={stageFilter} onValueChange={setStageFilter}>
            <SelectTrigger className="w-48">
              ...
              <SelectValue
                placeholder={t("quality.filter_by_stage", "Filter by stage")}
              />
            </SelectTrigger>
            <SelectContent>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {t("quality.check_type_name", "Name")}
                        </FormLabel>
                        <FormControl>
                          <Input
                            placeholder={t(
                              "quality.check_type_name_placeholder",
                              "Enter check type name",
                            )}
                            {...field}
                          />
                        </FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {t("quality.target_stage", "Target Stage")}
                        </FormLabel>
                        <Select
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t("quality.description", "Description")}
                      </FormLabel>
                      <FormControl>
                        <Textarea
                          placeholder={t(
                            "quality.check_type_description_placeholder",
                            "Enter check type description",
                          )}
                          {...field}
                        />
                      </FormControl>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {t("quality.check_types_list", "Quality Check Types")}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {filteredCheckTypes.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">
                {t(
                  "quality.create_first_check_type",
                  "Create your first quality check type to get started.",
                )}
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-4 mb-2">
                          ...
                          <Badge
                            variant={
                              checkType.isActive ? "default" : "secondary"
                            }
                          >
                            {checkType.isActive
                          ...
                          <Badge variant="outline">
                            {t(
                              `quality.stage_${checkType.targetStage}`,
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          {checkType.checklistItems &&
                            checkType.checklistItems.length > 0 && (
                              <div>
                                <span className="font-medium text-gray-700">
                                  {t(
                                    "quality.checklist_items",
                                    "Checklist Items",
                                  )}
                                  :
                                </span>
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {checkType.checklistItems.map(
                                    (item, index) => (
                                      <Badge
                                        key={index}
                                        variant="secondary"
