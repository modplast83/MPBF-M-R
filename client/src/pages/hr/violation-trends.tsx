...
export default function ViolationTrendsPage() {
  ...
  const generateMonthlyData = () => {
    ...
    return months.map(({ month, label }) => {
      const monthViolations = filteredViolations.filter(
        (v: HrViolation) => format(new Date(v.createdAt), "yyyy-MM") === month,
      );

      return {
        month: label,
        total: monthViolations.length,
        minor: monthViolations.filter(
          (v: HrViolation) => v.severity === "minor",
        ).length,
        major: monthViolations.filter(
          (v: HrViolation) => v.severity === "major",
        ).length,
        critical: monthViolations.filter(
          (v: HrViolation) => v.severity === "critical",
        ).length,
        attendance: monthViolations.filter(
          (v: HrViolation) => v.violationType === "attendance",
        ).length,
        conduct: monthViolations.filter(
          (v: HrViolation) => v.violationType === "conduct",
        ).length,
        safety: monthViolations.filter(
          (v: HrViolation) => v.violationType === "safety",
        ).length,
        performance: monthViolations.filter(
          (v: HrViolation) => v.violationType === "performance",
        ).length,
        policy: monthViolations.filter(
          (v: HrViolation) => v.violationType === "policy",
        ).length,
      };
    });
  };
  ...
  const severityData = {
    ...
    datasets: [
      {
        data: [
          filteredViolations.filter((v: HrViolation) => v.severity === "minor")
            .length,
          filteredViolations.filter((v: HrViolation) => v.severity === "major")
            .length,
  ...
  const trendPercentage =
    previousPeriod.length > 0
      ? ((Math.abs(trend) / previousPeriod.length) * 100).toFixed(1)
      : "0";

  const typeBreakdown = {
    attendance: filteredViolations.filter(
      (v: HrViolation) => v.violationType === "attendance",
    ).length,
    conduct: filteredViolations.filter(
      (v: HrViolation) => v.violationType === "conduct",
    ).length,
    safety: filteredViolations.filter(
      (v: HrViolation) => v.violationType === "safety",
    ).length,
    performance: filteredViolations.filter(
      (v: HrViolation) => v.violationType === "performance",
    ).length,
    policy: filteredViolations.filter(
      (v: HrViolation) => v.violationType === "policy",
    ).length,
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">
            {t("hr.violations.trends.title")}
          </h1>
          <p className="text-sm sm:text-base text-gray-600 mt-2">
            {t("hr.violations.trends.description")}
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold capitalize">
              {mostCommonType[0] || "None"}
            </div>
            <p className="text-xs text-gray-600 mt-1">
              {Number(mostCommonType[1]) || 0} incidents (
              {totalViolations > 0 && Number(mostCommonType[1]) > 0
                ? ((Number(mostCommonType[1]) / totalViolations) * 100).toFixed(
                    0,
                  )
                : "0"}
              %)
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {totalViolations > 0
                ? (
                    (filteredViolations.filter(
                      (v: HrViolation) => v.status === "resolved",
                    ).length /
                      totalViolations) *
                    100
                  ).toFixed(0) + "%"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {trend > 0 && (
              <div className="p-4 border border-red-200 bg-red-50 rounded-lg">
                ...
                <p className="text-sm text-red-600">
                  Violations have increased by{" "}
                  {Math.abs(Number(trendPercentage))}% compared to the previous
                  period. Consider reviewing current policies and training
                  programs.
                </p>
              </div>
            )}
            ...
            {criticalViolations > 0 && (
              <div className="p-4 border border-red-200 bg-red-50 rounded-lg">
                ...
                <p className="text-sm text-red-600">
                  {criticalViolations} critical violations require immediate
              ...
              mostCommonType[1] > totalViolations * 0.3 && (
                <div className="p-4 border border-yellow-200 bg-yellow-50 rounded-lg">
                  ...
                  <p className="text-sm text-yellow-600">
                    {mostCommonType[0] || "Unknown"} violations account for{" "}
                    {((mostCommonType[1] / totalViolations) * 100).toFixed(0)}%
                    of all cases. Consider targeted training for this area.
                  </p>
                </div>
              )}
          </div>
