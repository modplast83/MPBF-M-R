...
export default function EmployeeOfMonthPage() {
  ...
  const { data: employeeRecords, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiRequest(
        "GET",
        `${API_ENDPOINTS.EMPLOYEE_OF_MONTH}/year/${selectedYear}`,
      ),
  });
  ...
  const createEmployeeMutation = useMutation({
    mutationFn: (data: EmployeeOfMonthForm) => {
      const totalScore =
        (data.qualityScore + data.attendanceScore + data.productivityScore) / 3;
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [API_ENDPOINTS.EMPLOYEE_OF_MONTH],
      });
      toast({
  ...
  const getRankBadge = (rank: number | null) => {
    ...
    return <Badge className={`${color} border`}>#{rank}</Badge>;
  };
  ...
  return (
    <div className={`container mx-auto ${isMobile ? "p-3" : "p-3 sm:p-6"}`}>
      <div className={isMobile ? "mb-4" : "mb-6 sm:mb-8"}>
        <div
          className={`flex gap-4 ${isMobile ? "flex-col" : "flex-col sm:flex-row sm:justify-between sm:items-center"}`}
        >
          <div>
            <h1
              className={`font-bold text-gray-900 dark:text-gray-100 ${isMobile ? "text-xl" : "text-2xl sm:text-3xl"}`}
            >
              {t("hr.employee_of_month.title")}
            </h1>
            <p
              className={`text-gray-600 dark:text-gray-400 mt-2 ${isMobile ? "text-sm" : "text-sm sm:text-base"}`}
            >
                ...
                >
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {t("hr.employee_of_month.obligation_points")}
                        </FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {t("hr.employee_of_month.quality_score")}
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {t("hr.employee_of_month.attendance_score")}
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            {t("hr.employee_of_month.productivity_score")}
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {t("hr.employee_of_month.reward_amount")}
                        </FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-xs sm:text-sm">
                      {t("hr.employee_of_month.total_score")}
                    </span>
                    <span className="font-bold text-sm sm:text-base">
                      {employee.totalScore?.toFixed(1) || "0.0"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs sm:text-sm">
                ...
                {employeeRecords?.map((record: EmployeeOfMonth) => (
                  <TableRow key={record.id}>
                    ...
                    <TableCell className="font-bold">
                      {record.totalScore?.toFixed(1) || "0.0"}
                    </TableCell>
