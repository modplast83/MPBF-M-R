...
export default function OrdersIndex() {
  ...
  const filteredOrders = orders
    ?.filter((order) => {
      ...
      if (activeTab === "processing") {
        return (
          order.status === "processing" || order.status === "For Production"
        );
      }
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const message =
        data?.message || "Order and associated job orders deleted successfully";
    ...
    mutationFn: async (orderIds: number[]) => {
      const results = await Promise.allSettled(
        orderIds.map(async (id) => {
          ...
          if (!response.ok) {
            throw new Error(
              `Order #${id}: ${data.message || "Failed to delete"}`,
            );
          }
  ...
  const columns = [
    ...
    },
    {
      ...
      accessorKey: "customerId",
      cell: (row: { customerId: string }) =>
        getCustomerPlateDrawerCode(row.customerId),
    },
    {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                ...
                className={
                  row.status === "processing" ? "bg-secondary-100" : ""
                }
              >
    ...
    return (
      <div className="space-y-3">
        {filteredOrders.map((order) => (
          <Card
            key={order.id}
            className="overflow-hidden hover:shadow-md transition-all border-l-4 border-l-primary-500"
          >
                ...
                <CardHeader className="p-4 pb-3 flex flex-row justify-between items-start bg-gradient-to-r from-gray-50 to-white pl-12">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="material-icons text-sm text-primary-500">
                        receipt_long
                      </span>
                  ...
                  <div className="space-y-3">
                    ...
                    {order.note && (
                      <div className="bg-blue-50 p-3 rounded-lg">
                        ...
                        <p className="text-sm mt-1 text-blue-800 line-clamp-2">
                          {order.note}
                        </p>
                      </div>
                    )}
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className={isMobile ? "px-3" : ""}>
            ...
            >
              <TabsList
                className={`mb-4 ${isMobile ? "grid grid-cols-3 w-full h-auto" : ""}`}
              >
                <TabsTrigger
                  value="all"
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently delete order #{deletingOrder?.id} and all
                its associated data including: - Job orders - Rolls - Final
                products - Quality checks - SMS messages This action cannot be
                undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                order{selectedOrders.length > 1 ? "s" : ""}? This will
                permanently delete:
                <br />- Job orders
                <br />- Rolls
                <br />- Final products
