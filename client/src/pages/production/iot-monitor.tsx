...
export default function IoTMonitor() {
  ...
  const handleCreateSensor = () => {
    const sensorData = {
      ...
      warningThreshold: newSensorData.warningThreshold
        ? parseFloat(newSensorData.warningThreshold)
        : null,
      criticalThreshold: newSensorData.criticalThreshold
        ? parseFloat(newSensorData.criticalThreshold)
        : null,
      isActive: true,
    };
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold">
            {t("iot.title", "IoT Monitor")}
          </h1>
          <p className="text-gray-600 mt-1">
            {t("iot.description", "Real-time machine monitoring and alerts")}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={isAddSensorOpen} onOpenChange={setIsAddSensorOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  {t("iot.add_new_sensor", "Add New Sensor")}
                </DialogTitle>
                <DialogDescription>
                  {t(
                    "iot.configure_sensor",
              ...
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("iot.select_machine", "Select machine")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="temperature">
                        {t("iot.sensor_types.temperature", "Temperature")}
                      </SelectItem>
                      <SelectItem value="pressure">
                        {t("iot.sensor_types.pressure", "Pressure")}
                      </SelectItem>
                      <SelectItem value="speed">
                        {t("iot.sensor_types.speed", "Speed")}
                      </SelectItem>
                      <SelectItem value="vibration">
                        {t("iot.sensor_types.vibration", "Vibration")}
                      </SelectItem>
                      <SelectItem value="energy">
                        {t("iot.sensor_types.energy", "Energy")}
                      </SelectItem>
                      <SelectItem value="status">
                        {t("iot.sensor_types.status", "Status")}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    placeholder={t(
                      "iot.sensor_name_placeholder",
                      "e.g., Main Motor Temperature",
                    )}
                  />
                </div>
                <div className="grid gap-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label htmlFor="warning">
                      {t("iot.warning_threshold", "Warning Threshold")}
                    </Label>
                    <Input
                      id="warning"
                      type="number"
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="critical">
                      {t("iot.critical_threshold", "Critical Threshold")}
                    </Label>
                    <Input
                      id="critical"
                      type="number"
              ...
              <DialogFooter>
                ...
                >
                  {createSensorMutation.isPending
                    ? t("iot.creating", "Creating...")
                    : t("iot.create_sensor", "Create Sensor")}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      {t("iot.current", "Current")}: {alert.currentValue} |{" "}
                      {t("iot.threshold", "Threshold")}: {alert.thresholdValue}
                    </p>
                  </div>
                  <div className="flex gap-2">
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList>
          <TabsTrigger value="overview">
            {t("iot.overview", "Overview")}
          </TabsTrigger>
          <TabsTrigger value="sensors">
            {t("iot.sensors", "Sensors")}
          </TabsTrigger>
          <TabsTrigger value="analytics">
            {t("iot.analytics", "Analytics")}
          </TabsTrigger>
        </TabsList>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="capitalize">
                        {t(
                          `iot.sensor_types.${sensor.sensorType}`,
                          sensor.sensorType,
                        )}
                      </span>
                    </div>
        ...
        <TabsContent value="sensors" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                {t("iot.filter_by_machine", "Filter by Machine")}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <Select
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="all">
                    {t("iot.all_machines", "All Machines")}
                  </SelectItem>
                  {machines.map((machine: any) => (
                    <SelectItem key={machine.id} value={machine.id}>
                      {machine.name}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription>
                          {sensor.sensorType} sensor on machine{" "}
                          {sensor.machineId}
                        </CardDescription>
                      </div>
                    </div>
