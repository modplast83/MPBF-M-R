...
import {
  AbaCalculator,
  AbaPrintTemplate,
} from "@/components/production/aba-calculator";
...
export default function MixMaterialsPage() {
  ...
  const getFilteredMixMaterials = () => {
    ...
    return mixMaterials.filter((mix) => {
      ...
      if (filterMode === "date" && filterDate) {
        ...
        return (
          format(mixDate, "yyyy-MM-dd") === format(filterDateObj, "yyyy-MM-dd")
        );
      }
      ...
      if (filterMode === "week" && filterWeek) {
        ...
        const startDate = startOfWeek(
          new Date(parseInt(year), 0, 1 + (parseInt(week) - 1) * 7),
        );
  ...
  const mixColumns = [
    ...
    },
    {
      ...
      cell: (row: { totalQuantity: number | null }) =>
        row.totalQuantity?.toFixed(2) || "0.00",
    },
    {
      header: "Screw",
  ...
  const preparePrintData = async (mixId: number) => {
    try {
      ...
      const itemsResponse = await fetch(
        `${API_ENDPOINTS.MIX_MATERIALS}/${mixId}/items`,
      );
  ...
  const PrintableLabel = () => {
    ...
    const { mix, items, totalWeight, materialNames, userData } =
      printableData as PrintData;
    ...
    return (
      <div
        ...
        style={{ width: "3in", height: "5in", padding: "0.25in" }}
      >
        <div
          style={{
            fontWeight: "bold",
        ...
        >
          ...
          <div>
            {t("production.mix_materials.date")}:{" "}
            {formatDateString(mix.mixDate)}
          </div>
        </div>
        <div style={{ marginBottom: "10px" }}>
          <div>
            {t("production.mix_materials.operator")}:{" "}
            {getPrintOperatorName(mix.mixPerson)}
          </div>
          <div>
            {t("production.mix_materials.total_weight")}:{" "}
            {totalWeight.toFixed(2)} kg
          </div>
          <div>
            {t("production.mix_materials.screw")}: {mix.mixScrew || "-"}
          </div>
        </div>
          ...
          <tbody>
            {items.map(
              (item: {
                ...
                const materialName =
                  materialNames[item.rawMaterialId] ||
        ...
        <div
          style={{ textAlign: "center", fontSize: "8pt", marginTop: "0.1in" }}
        >
    ...
    if (!filteredMixMaterials || filteredMixMaterials.length === 0) {
      return (
        <div className="text-center py-12 px-4 bg-gray-50 rounded-lg">
          <span className="material-icons text-gray-300 text-4xl mb-3 block">
            science
          </span>
          <p className="text-gray-500 text-sm">
            {t("production.mix_materials.no_mixes")}
          </p>
        </div>
      );
    }
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="flex items-center justify-center w-8 h-8 bg-primary-100 rounded-full">
                    <span className="material-icons text-sm text-primary-600">
                      science
                    </span>
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {mix.totalQuantity?.toFixed(2) || "0.00"} kg
                  </p>
                </div>
              </div>
              ...
              {mix.mixScrew && (
                <div className="bg-blue-50 p-3 rounded-lg mb-3">
                  <p className="text-xs text-blue-600 mb-1">
                    {t("production.mix_materials.screw")}
                  </p>
      ...
      <div className="mb-4">
        <div className="text-center py-4 px-2 bg-gray-50 rounded-md mb-4">
          <p className="text-gray-500">
            ABA configurations have been removed from the database
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div
        className={`${isMobile ? "flex flex-col space-y-3" : "flex justify-between items-center"} mb-4 sm:mb-6`}
      >
        <h1
          className={`${isMobile ? "text-xl" : "text-2xl"} font-bold text-primary-700`}
        >
        ...
        {canCreate && (
          ...
          >
            <DialogTrigger asChild>
              <Button
                className={`${isMobile ? "w-full justify-center" : "flex items-center gap-1"}`}
              >
            ...
            <DialogContent
              className={
                isMobile
                  ? "max-w-[95vw] max-h-[95vh] p-3 sm:p-6 overflow-y-auto"
                  : "sm:max-w-[550px]"
              }
            >
              <DialogHeader>
      ...
      <Tabs defaultValue="mix_materials" className="w-full">
        <TabsList
          className={`grid w-full mb-4 ${isMobile ? "grid-cols-1 gap-1 h-auto p-1" : "grid-cols-3"}`}
        >
          <TabsTrigger
            value="mix_materials"
            className={isMobile ? "w-full justify-start py-3 px-4" : ""}
          >
                ...
                {!isMobile ? (
                  ...
                  <div className="w-full flex gap-2">
                    <select
                      ...
                      onChange={(e) => {
                        setFilterMode(
                          e.target.value as "all" | "date" | "week" | "month",
                        );
                      }}
                    >
            ...
            <CardContent>
              ...
              <div className="text-center py-8 px-4 bg-gray-50 rounded-md">
                <p className="text-gray-600">
                  ABA Calculator has been removed from the system
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="aba_config">
          <Card>
            <CardHeader className={`${isMobile ? "p-4 pb-2" : "pb-0"}`}>
              ...
              <div
                className={`${isMobile ? "flex flex-col space-y-3" : "flex items-center justify-between"} mt-1`}
              >
                <CardDescription className={isMobile ? "text-sm" : ""}>
                  {t("production.aba_calculator.drag_help")}
                </CardDescription>
                <div
                  className={`${isMobile ? "flex flex-col space-y-2" : "flex space-x-2"}`}
                >
                  {selectedConfigId && (
                    <Button
                      variant="outline"
            ...
            <CardContent>
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <h3 className="font-medium text-sm">
                          {t("production.aba_calculator.editing_config")}
                        </h3>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const newName = prompt(
                            t("production.aba_calculator.update_config_name"),
                            configName,
                          );
                          if (newName) {
                            const newDesc = prompt(
                              t("production.aba_calculator.update_config_desc"),
                              configDescription,
                            );
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2">
                  <div className="text-center py-8 px-4 bg-gray-50 rounded-md">
                    <p className="text-gray-600">
                      ABA Materials component has been removed
                    </p>
                  </div>
                </div>
                <div className="lg:col-span-1">
                  <div className="text-center py-4 px-2 bg-gray-50 rounded-md">
                    <p className="text-gray-500">
                      ABA configurations have been removed
                    </p>
                  </div>
                </div>
              </div>
      ...
      <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        <DialogContent
          className={
            isMobile
              ? "max-w-[95vw] p-4 overflow-y-auto max-h-[90vh]"
              : "sm:max-w-[750px]"
          }
        >
          <DialogHeader>
      ...
      <div className="hidden print:block print:m-0 print:p-0">
        {abaCalculationData ? (
          <AbaPrintTemplate data={abaCalculationData} />
        ) : (
          <PrintableLabel />
        )}
      </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("common.delete_confirmation_message", {
                item: t("production.mix_materials.title"),
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
