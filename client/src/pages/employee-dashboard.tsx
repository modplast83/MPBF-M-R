...
export default function EmployeeDashboard() {
  ...
  return (
    <div className="container mx-auto py-3 sm:py-6 px-4 sm:px-6">
      ...
      >
        <div>
          <h1 className={`${isMobile ? "text-2xl" : "text-3xl"} font-bold`}>
            {t("user_dashboard.welcome", "Welcome")},{" "}
        ...
        <Button
          variant="outline"
          onClick={() => setLocation("/setup/users")}
      ...
      {/* Quick Status Cards */}
      <div
        className={`grid ${isMobile ? "grid-cols-2 gap-3" : "grid-cols-1 md:grid-cols-4 gap-6"} mb-6`}
      >
        <Card>
          <CardContent className={`${isMobile ? "p-4" : "p-6"}`}>
            <div className="flex items-center justify-between pt-[0px] pb-[0px] pl-[2px] pr-[2px] ml-[-19px] mr-[-19px]">
              ...
              <Clock
                className={`${isMobile ? "h-5 w-5 mx-auto" : "h-8 w-8"} text-blue-500`}
              />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className={`${isMobile ? "p-4" : "p-6"}`}>
            <div className="flex items-center justify-between ml-[-14px] mr-[-14px] pl-[0px] pr-[0px]">
              ...
              <TrendingUp
                className={`${isMobile ? "h-5 w-5 mx-auto" : "h-8 w-8"} text-green-500`}
              />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className={`${isMobile ? "p-4" : "p-6"}`}>
            <div className="flex items-center justify-between ml-[-6px] mr-[-6px]">
              ...
              <Calendar
                className={`${isMobile ? "h-5 w-5 mx-auto" : "h-8 w-8"} text-purple-500`}
              />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className={`${isMobile ? "p-4" : "p-6"}`}>
            ...
            >
              ...
              <AlertTriangle
                className={`${isMobile ? "h-5 w-5 mx-auto" : "h-8 w-8"} text-red-500`}
              />
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList
          className={`grid w-full ${isMobile ? "grid-cols-2 h-auto" : "grid-cols-4"}`}
        >
          <TabsTrigger
            value="overview"
          ...
          >
            {isMobile
              ? t("user_dashboard.overview_short", "Overview")
          ...
          >
            {isMobile
              ? t("user_dashboard.attendance_short", "Attend.")
              : t("user_dashboard.attendance", "Attendance")}
          </TabsTrigger>
          <TabsTrigger value="tasks" className={isMobile ? "text-xs py-2" : ""}>
            {isMobile
              ? t("user_dashboard.tasks_short", "Tasks")
              : t("user_dashboard.tasks", "Tasks")}
          </TabsTrigger>
          <TabsTrigger
            value="schedule"
            className={isMobile ? "text-xs py-2" : ""}
          >
            {isMobile
              ? t("user_dashboard.schedule_short", "Schedule")
          ...
          >
            ...
            <Card>
              <CardHeader className={isMobile ? "pb-3" : ""}>
                <CardTitle
                  className={`flex items-center ${isMobile ? "text-base" : ""}`}
                >
                  <Clock
              ...
              <CardContent className={isMobile ? "pt-0" : ""}>
                <div className={`space-y-${isMobile ? "3" : "4"}`}>
                  ...
                  <div className={`pt-${isMobile ? "3" : "4"} border-t`}>
                    <p
                      className={`${isMobile ? "text-xs" : "text-sm"} text-gray-600 mb-2`}
                    >
                      {t("user_dashboard.location", "Location")}
                    </p>
                    <div
                      className={`flex items-center ${isMobile ? "text-xs" : "text-sm"} text-gray-500`}
                    >
                      <MapPin
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-8 text-gray-500">
                {t(
                  "user_dashboard.no_recent_activities",
        ...
        <TabsContent value="attendance" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("user_dashboard.monthly_summary", "Monthly Summary")}
              </CardTitle>
              <CardDescription>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-600">
                    {t("user_dashboard.days_absent", "Days Absent")}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-blue-600">176</p>
                  <p className="text-sm text-gray-600">
                    {t("user_dashboard.total_hours", "Total Hours")}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-purple-600">8</p>
                  <p className="text-sm text-gray-600">
                    {t("user_dashboard.overtime_hours", "Overtime Hours")}
                  </p>
                </div>
              </div>
            </CardContent>
